...
from model import N_KEYPOINTS, WINDOW_SIZE, load_checkpoint
...
def main():
    ...
    n_rx = int(ckpt.get("config", {}).get("n_rx", 1))
    print(f"[INFO] Model dung {n_rx} RX (subcarriers={ckpt.get('config',{}).get('subcarriers','?')})")

    base = CSIPoseDataset(args.data, n_rx=n_rx)
    if len(base) == 0:
