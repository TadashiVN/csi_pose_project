...
Phím: 1=walk 2=stand 3=sit 4=fall 5=empty 6=arms_horizontal 7=arms_up | s=ghi x=dừng q=thoát
Chạy:
  python data_collection.py --ports COM3 COM9 --cam http://192.168.51.3:4747/video --out dataset/raw/session_01.json

LƯU Ý động tác tay (6,7):
  - Cần ÍT NHẤT 1 RX đặt cao (ngang vai/đầu) thì tay giơ lên mới làm nhiễu tín hiệu.
  - Camera phải lọt CẢ tay giơ cao; người hướng về camera, đủ sáng (MediaPipe phải thấy tay).
  - Giữ YÊN tư thế trong lúc ghi (đây là trạng thái tĩnh, không phải động tác).
"""
...
ACTIVITY_MAP = {
    "1": "walk", "2": "stand", "3": "sit", "4": "fall", "5": "empty",
    "6": "arms_horizontal",   # đứng yên, hai tay giơ ngang (T-pose)
    "7": "arms_up",           # đứng yên, hai tay giơ thẳng lên trời
}
...
def main():
    ...
    print("  [6]arms_horizontal  [7]arms_up   (giơ 2 tay, giữ yên)")
    ...
    try:
        ...
        if samples:
            print("[INFO] Đang nén và lưu (.npz) — chờ chút...")
            csi_arr = np.array([s["csi"]       for s in samples], dtype=np.float32)
            kps_arr = np.array([s["keypoints"] for s in samples], dtype=np.float32)
            ...
            npz_path, meta_path, dist = save_session_npz(
                args.out, csi_arr, kps_arr, acts, tss, len(args.ports), meta)
            print(f"[SAVED] {npz_path}")
            print(f"[SAVED] {meta_path}  (metadata)")
            print("  Distribution:")
            for act, cnt in sorted(dist.items()):
                print(f"    {act:16s}: {cnt:4d}  {'█'*(cnt//50)}")
        else:
            print("[!] Không có data.")
